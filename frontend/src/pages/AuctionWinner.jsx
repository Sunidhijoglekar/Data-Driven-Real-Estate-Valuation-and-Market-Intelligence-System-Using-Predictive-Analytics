...
import {
  Trophy, Sparkles, MapPin, CheckCircle2,
  Award, Gavel, FileText, ArrowLeft
} from 'lucide-react';
    ...
    try {
      ...
      if (aucData) {
        ...
        try {
          ...
          console.warn('Winner report fetch warning:', e);
        }
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center space-y-4">
        ...
        <p className="text-sm font-bold text-slate-600">Generating Official Property Sale Summary & Valuation Report...</p>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
        ...
        <div className="p-6 sm:p-8 space-y-8">
          ...
          {aiReport && (
            <div className="bg-gradient-to-r from-blue-900 via-slate-900 to-indigo-950 p-6 sm:p-8 rounded-3xl text-white space-y-4 shadow-xl border border-blue-800/50">
              <div className="flex items-center gap-2 text-blue-300 text-xs font-bold uppercase tracking-wider">
                <Sparkles className="w-4 h-4 text-blue-400" /> Property Valuation & Sale Analytics Report
              </div>
          ...
          <div className="flex justify-center pt-2">
            <button
              onClick={() => navigate('/my-auctions')}
              className="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-lg transition-all cursor-pointer"
            >
