...
import {
  Gavel, Clock, Users, ShieldCheck, ArrowLeft, MapPin, CheckCircle2,
  AlertCircle, Lock, Ticket, Snowflake, PauseCircle
} from 'lucide-react';
...
export default function LiveAuction({ user }) {
  ...
  const authorizedParticipant = auction?.participants?.find(
    p => p.buyer_id === buyerId || p.buyer_email === buyerId
  );
  const approvedRegistration = auction?.registrations?.find(
    r => (r.buyer_id === buyerId || r.buyer_email === buyerId) && r.status === 'APPROVED'
  );
  const isAuthorizedParticipant = Boolean(authorizedParticipant || approvedRegistration);
  const isLive = auction?.status === 'LIVE';
  const auctionExpired = Boolean(auction?.auction_end && new Date(auction.auction_end).getTime() <= Date.now());

  const handlePlaceBid = async (e) => {
    e.preventDefault();
    if (!auction) return;

    if (auction.status !== 'LIVE' || auctionExpired) {
      setStatusMsg({
        type: 'error',
        text: auctionExpired ? 'This auction has reached its scheduled end time.' : `Bidding is currently ${String(auction.status).toLowerCase().replaceAll('_', ' ')}.`
      });
      return;
    }
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-xl text-center space-y-5">
          ...
          <div className="pt-2 flex justify-center gap-3">
            <button
              ...
              className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-lg transition-all cursor-pointer"
            >
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      {!isLive && !isFrozen && !isPaused && (
        <div className="bg-slate-100 text-slate-700 p-5 rounded-2xl border border-slate-200 shadow-sm">
          <h3 className="font-heading text-sm font-black uppercase tracking-wider">Bidding is not live</h3>
          <p className="text-xs mt-1">
            The seller controls when bidding starts. Current auction status:
            <strong className="ml-1">{auction?.status?.replaceAll('_', ' ')}</strong>.
          </p>
        </div>
      )}
      {auctionExpired && isLive && (
        <div className="bg-rose-50 text-rose-800 p-5 rounded-2xl border border-rose-200">
          <h3 className="font-heading text-sm font-black uppercase tracking-wider">Auction time expired</h3>
          <p className="text-xs mt-1">No new bids should be placed. The seller can finalize the transaction.</p>
        </div>
      )}
      {isFrozen && (
          ...
          <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-lg space-y-5">
            <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
              ...
              <span className="text-[11px] font-bold text-emerald-600 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200">
                {authorizedParticipant || approvedRegistration ? '1 Token Active' : 'No Token'}
              </span>
            </div>
            ...
            {isFrozen ? (
            ...
            ) : !isLive || auctionExpired ? (
              <div className="p-4 bg-slate-100 rounded-2xl text-center text-xs font-bold text-slate-600">
                Bidding is unavailable until the seller starts a live auction.
              </div>
            ) : (
              <form onSubmit={handlePlaceBid} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={bidding}
                  className="w-full py-3.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-lg shadow-blue-500/25 flex items-center justify-center gap-2 transition-all cursor-pointer"
                >
                  {bidding ? (
                    <>Placing Bid...</>
                  ) : (
