...
import {
  Gavel, Search, Users, Flame, Clock, Trophy, CheckCircle2,
  AlertCircle, ArrowLeft, Building2, MapPin, ShieldCheck, Ticket
} from 'lucide-react';
...
export default function LiveAuctionsDashboard({ user }) {
  ...
  const getAuctionDisplayStatus = (auc) => {
    ...
    if (auc.status === 'REGISTRATION_CLOSED') return 'Registration Closed';
    if (now < start || auc.status === 'REGISTRATION_OPEN' || auc.status === 'UPCOMING') return 'Registration Open';
    return 'Live';
  };
  ...
  const getAuctionComputedStatus = (auc) => {
    ...
    if (now < start || auc.status === 'REGISTRATION_OPEN' || auc.status === 'UPCOMING') return 'UPCOMING';
    return 'LIVE';
  };

  // Helper for format time remaining
  const getTimeRemaining = (targetTimeStr) => {
    if (!targetTimeStr) return 'Ended';
    const total = Date.parse(targetTimeStr) - Date.parse(now);
      ...
      <div className="bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 rounded-3xl p-6 sm:p-8 text-white shadow-xl border border-blue-800/40 relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-2 z-10 max-w-2xl">
          ...
          <p className="text-slate-300 text-xs sm:text-sm">
            Discover verified luxury, residential, and commercial properties up for live digital bidding. Track real-time highest bids, participant counts, and market valuations.
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 w-full md:w-auto text-xs font-semibold">
            ...
            <select
              ...
              className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white outline-none cursor-pointer"
            >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredAuctions.map(auc => {
            ...
            const isRegOpen = displayStatus === 'Registration Open';
            const isLive = displayStatus === 'Live';
            const isRegClosed = displayStatus === 'Registration Closed';

            // Calculate timing based on status
            const timeRem = isClosed
              ? 'Ended'
              : isRegOpen
              ? getTimeRemaining(auc.auction_start)
              : getTimeRemaining(auc.auction_end);
                ...
                <div>
                  ...
                  <div className="relative h-48 bg-slate-900 overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3">
                      ...
                      {isRegClosed && (
                        <span className="px-3 py-1 bg-slate-700 text-white font-black text-[10px] uppercase tracking-wider rounded-full shadow-md flex items-center gap-1 border border-slate-600">
                  ...
                  <div className="p-5 space-y-4">
                    ...
                    <div className="space-y-2 text-xs border-t border-slate-100 pt-3">
                      <div className="flex justify-between items-center text-slate-600">
                        ...
                        <strong className={`font-bold ${isLive ? 'text-amber-600' : isRegOpen ? 'text-blue-600' : 'text-slate-500'}`}>
                          {displayStatus}
                        </strong>
                      </div>
                      ...
                      <div className="flex justify-between items-center text-slate-600">
                        <span>
                          {isLive ? 'Bidding Ends In:' : isRegOpen ? 'Auction Starts In:' : isRegClosed ? 'Starts In:' : 'Remaining Time:'}
                        </span>
                        <strong className={`font-mono ${isClosed ? 'text-slate-500' : 'text-slate-800'}`}>
                          {timeRem}
                        </strong>
                      </div>
                ...
                <div className="p-5 pt-0 space-y-2">
                  ...
                  {hasAuctionPass ? (
                    ...
                    >
                      ...
                      {isLive ? 'Enter Live Bidding' : 'Join Auction Room'}
                    </button>
                  ) : isRegOpen ? (
                    <div className="p-3 bg-blue-50 border border-blue-200 rounded-xl text-center space-y-2">
                      ...
                      <button
                        ...
                        className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white font-black text-xs uppercase tracking-wider rounded-lg shadow-sm flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                      >
                        <Ticket className="w-3.5 h-3.5" /> Get Auction Pass (Token)
                      </button>
                    </div>
                  ) : isLive ? (
                    <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-center space-y-1">
                      <p className="text-xs font-bold text-amber-800 flex items-center justify-center gap-1">
                        ...
                        Live Bidding in Progress (Pass Required)
                      </p>
                      <p className="text-[11px] text-amber-700">
                        Registration for this live auction is closed. Spectator mode available.
                      </p>
                    </div>
                  ) : isRegClosed ? (
                    <div className="p-3 bg-slate-100 border border-slate-200 rounded-xl text-center space-y-1">
                      <p className="text-xs font-bold text-slate-700 flex items-center justify-center gap-1">
                        <Lock className="w-3.5 h-3.5 text-slate-500 shrink-0" />
                        Registration Closed
                      </p>
                      <p className="text-[11px] text-slate-500">
                        Registration has ended. Awaiting live bidding start.
                      </p>
                    </div>
                  ) : (
                    <div className="p-3 bg-slate-100 border border-slate-200 rounded-xl text-center space-y-1">
                      ...
                      <p className="text-[11px] text-slate-500">
                        This property auction has concluded.
                      </p>
                    </div>
                  )}

                  {/* Secondary Action: View Auction or Summary */}
                  {isClosed ? (
                    <button
                      onClick={() => navigate(`/auction-result/${auc.auction_id}`)}
                      className="w-full py-2 bg-slate-800 hover:bg-slate-900 text-white font-bold text-xs rounded-xl transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                      <CheckCircle2 className="w-3.5 h-3.5" /> View Auction Result
                    </button>
                  ) : (
                    <button
                      onClick={() => navigate(`/auction/${prop.id}/live`)}
                    ...
                    >
                      {hasAuctionPass ? 'Enter Bidding Room' : 'View Auction (Spectate)'}
                    </button>
                  )}
