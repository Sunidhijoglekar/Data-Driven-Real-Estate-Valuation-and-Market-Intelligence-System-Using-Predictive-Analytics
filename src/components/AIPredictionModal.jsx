  ...
  useEffect(() => {
    async function loadValuationAndInsights() {
      ...
      try {
        ...
        console.error('Error fetching AI valuation:', err);
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl max-w-3xl w-full border border-slate-200 shadow-2xl overflow-hidden my-8 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="bg-gradient-to-r from-blue-900 via-slate-900 to-indigo-900 text-white p-6 relative">
          <button
            onClick={onClose}
            className="absolute top-5 right-5 p-1.5 rounded-full bg-white/10 text-slate-300 hover:text-white hover:bg-white/20 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <div className="flex items-center gap-2 text-blue-400 text-xs font-bold uppercase tracking-wider mb-2">
            ...
            AI Valuation & Market Intelligence Report
          </div>
        ...
        <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          {loading ? (
            <div className="py-16 text-center space-y-3">
              ...
              <p className="text-xs font-bold text-slate-700">Running Random Forest, XGBoost, ARIMA & LSTM Models...</p>
              <p className="text-[11px] text-slate-400">Synthesizing Gemini AI market trend insights...</p>
            </div>
          ) : (
            <>
              {/* 1. Current Price Prediction Comparison */}
              <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-heading font-bold text-slate-900 text-sm flex items-center gap-2">
                    <Cpu className="w-4 h-4 text-blue-600" />
                    Current Valuation (Random Forest vs XGBoost)
                  </h3>
                  <span className="text-[10px] bg-emerald-100 text-emerald-800 font-extrabold px-2.5 py-0.5 rounded-full border border-emerald-200">
                    Selected Model: XGBoost (R² = 0.9635)
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-center">
                  ...
                  <div className="p-3 bg-white rounded-xl border border-slate-200">
                    <span className="text-[10px] uppercase font-bold text-slate-400 block">Random Forest Regressor</span>
                  ...
                  <div className="p-3 bg-blue-50 rounded-xl border border-blue-200">
                    <span className="text-[10px] uppercase font-bold text-blue-600 block">XGBoost (Winner)</span>
              ...
              <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-heading font-bold text-slate-900 text-sm flex items-center gap-2">
                    ...
                    Time Series Price Forecasting (ARIMA vs LSTM)
                  </h3>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-xs text-left">
                    ...
                    <tbody className="divide-y divide-slate-200">
                      <tr>
                        <td className="p-2.5 font-bold text-slate-700">ARIMA</td>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-3 bg-emerald-50 rounded-2xl border border-emerald-200 text-center">
                  <span className="text-[10px] font-bold text-emerald-800 uppercase block">Investment Score</span>
