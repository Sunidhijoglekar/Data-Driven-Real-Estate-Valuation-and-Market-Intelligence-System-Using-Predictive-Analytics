...
import {
  Gavel, Clock, ShieldCheck, ArrowLeft, Building2, MapPin, Sparkles, CheckCircle2,
...
export default function JoinAuction({ user }) {
  ...
  const [notes, setNotes] = useState('Interested in purchasing property via invitation auction.');
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Try fetching auction by ID or property ID
      try {
        ...
        // Fallback to property ID lookup
  ...
  const isRejected = userReg && userReg.status === 'REJECTED';
  const isRegOpen = auction?.status === 'REGISTRATION_OPEN';
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div className="bg-white/80 backdrop-blur-md rounded-3xl border border-slate-200/80 shadow-xl overflow-hidden">
        ...
        <div className="relative h-64 sm:h-72 w-full overflow-hidden bg-slate-900">
          ...
          <div className="absolute bottom-6 left-6 right-6 text-white space-y-2">
            <div className="flex items-center gap-2 text-blue-300 text-xs font-semibold">
              <MapPin className="w-3.5 h-3.5" /> {property?.locality || 'Prime Locality'}, {property?.city || 'Mumbai'}
            </div>
        ...
        <div className="p-6 sm:p-8 space-y-6">
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-2xl p-6 space-y-4">
              ...
              <div className="pt-2 flex flex-col sm:flex-row gap-3">
                <button
                  ...
                  className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-lg shadow-blue-500/25 flex items-center justify-center gap-2 transition-all"
                >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-700">
                    Your token request was submitted to the seller on {new Date(userReg.requested_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}. You will receive a notification once approved.
                  </p>
                </div>
              </div>
            </div>
          ...
          {/* STATE 3: REGISTRATION CLOSED & NOT APPROVED (MANDATORY EXACT REQUIREMENT) */}
          {!isApproved && !isPending && !isRegOpen && (
            <div className="bg-slate-100 border border-slate-300 rounded-2xl p-8 text-center space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500 max-w-md mx-auto pt-1">
                  The seller has manually stopped registration. Only pre-authorized buyers with an issued Auction Token can enter and participate in live bidding.
                </p>
              </div>
            </div>
          )}
            ...
            <form onSubmit={handleRequestToken} className="space-y-5 bg-slate-50/70 p-6 rounded-2xl border border-slate-200">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1 sm:col-span-2">
                  <label className="text-[11px] font-bold uppercase tracking-wider text-slate-600 flex items-center gap-1">
                    <Phone className="w-3.5 h-3.5 text-blue-600" /> Phone Number for Seller Verification
                  </label>
                  <input
                    type="tel"
                    value={phone}
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full sm:w-auto px-8 py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-lg shadow-blue-500/25 flex items-center justify-center gap-2 transition-all"
                >
                  {submitting ? (
                    <>Submitting Request...</>
                  ) : (
